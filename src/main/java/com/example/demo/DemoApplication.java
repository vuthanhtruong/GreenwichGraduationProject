...
public class DemoApplication {
    ...
    public static void main(String[] args) {
        ...
        try {
            em.getTransaction().begin();
            addDefaultStaffs(em, passwordEncoder);
            em.getTransaction().commit();
        } catch (Exception e) { rollback(em); e.printStackTrace(); }

        try {
            em.getTransaction().begin();
            addDefaultDeputyStaff(em, passwordEncoder);
            em.getTransaction().commit();
        } catch (Exception e) { rollback(em); e.printStackTrace(); }
        ...
        // === Bulk seed theo danh sách bạn gửi (Subjects + TuitionByYear) ===
        try {
            em.getTransaction().begin();
    ...
    // ===================== STAFFS + ADMIN =====================
    private static void addDefaultStaffs(EntityManager em, PasswordEncoder passwordEncoder) {
        List<Persons> toAdd = new ArrayList<>();
        Majors gbh, gch;
        try {
            gbh = em.createQuery("SELECT m FROM Majors m WHERE m.majorId = :id", Majors.class)
                    .setParameter("id", "GBH").getSingleResult();
            gch = em.createQuery("SELECT m FROM Majors m WHERE m.majorId = :id", Majors.class)
                    .setParameter("id", "GCH").getSingleResult();
        } catch (NoResultException e) {
        ...
        admin.setId("Admin"); admin.setFirstName("Admin"); admin.setLastName("User");
        admin.setEmail("admin@example.com"); admin.setPhoneNumber("0390000000");
        admin.setBirthDate(LocalDate.of(1980,1,1)); admin.setGender(Gender.OTHER);
        admin.setCountry("Vietnam"); admin.setProvince("Hanoi"); admin.setCity("Hanoi");
        admin.setDistrict("Ba Dinh"); admin.setWard("Ngoc Ha"); admin.setStreet("10 Ngoc Ha");
        admin.setPostalCode("100000");

        toAdd.addAll(List.of(staff1, staff2, admin));
        ...
        for (Persons p : toAdd) {
            ...
            try {
                ...
                exists = true;
            } catch (NoResultException e) { exists = false; }
            ...
            if (!exists) {
                ...
                auth.setPerson(p);
                auth.setPassword(p instanceof Admins ? "Admin123" : "Anhnam123");
                em.persist(auth);
    ...
    // ===================== DEPUTY STAFF =====================
    private static void addDefaultDeputyStaff(EntityManager em, PasswordEncoder passwordEncoder) {
        Admins admin;
        try {
            admin = em.createQuery("SELECT a FROM Admins a WHERE a.id = :id", Admins.class)
                    .setParameter("id", "Admin").getSingleResult();
        } catch (NoResultException e) {
            admin = new Admins();
            admin.setId("Admin"); admin.setFirstName("Admin"); admin.setLastName("User");
            admin.setEmail("admin@example.com"); admin.setPhoneNumber("0390000000");
            ...
            em.persist(admin);
            Authenticators auth = new Authenticators();
            auth.setPersonId(admin.getId()); auth.setPerson(admin); auth.setPassword("Admin123");
            em.persist(auth);
        }
        ...
        try {
            ...
            Authenticators auth = new Authenticators();
            auth.setPersonId(deputy.getId()); auth.setPerson(deputy); auth.setPassword("Anhnam123");
            em.persist(auth);
    ...
    private static void addDefaultMajorSubjects(EntityManager em) {
        Staffs creator = em.createQuery("SELECT s FROM Staffs s WHERE s.id = :id", Staffs.class)
                .setParameter("id", "vuthanhtruong")
                .getSingleResult();
        ...
        for (String majorId : majorIds) {
            Majors major;
            try {
                major = em.createQuery("SELECT m FROM Majors m WHERE m.majorId = :id", Majors.class)
                        .setParameter("id", majorId).getSingleResult();
            } catch (NoResultException e) {
    ...
    private static void addDefaultMinorSubjects(EntityManager em) {
        DeputyStaffs deputy = em.createQuery("SELECT d FROM DeputyStaffs d WHERE d.id = :id", DeputyStaffs.class)
                .setParameter("id", "deputy001").getSingleResult();
    ...
    private static void bulkSeedSubjectsAndTuition(EntityManager em) {
        ...
        // === START OF FULL DATASET YOU PROVIDED ===
        // NOTE: tuition values use dot as decimal separator; zeros/blank -> no TuitionByYear record
        // Format: seeds.add(new SubjectSeed("ID","Name", creditsIntOrNull, tuitionDoubleOrNull));
        seeds.add(new SubjectSeed("1244","Business Skills for E-comerce",15, 6540000.0));
        ...
        seeds.add(new SubjectSeed("ENR003","English level 0",0, 11950000.0)); // theo cột cuối
        ...
        for (SubjectSeed seed : uniqueSeeds) {
            ...
            if (exists) {
                ...
                if (subj == null) {
                    // Could be MinorSubjects or proxy of Subjects; try generic load and convert
                ...
                if (subj != null) {
                    ...
                    // If existing but not MajorSubjects, skip creating another
            ...
            if (seed.tuition != null && seed.tuition > 0) {
                TuitionByYearId tyId = new TuitionByYearId(seed.id, currentYear);
                TuitionByYear ty = em.find(TuitionByYear.class, tyId);
                if (ty == null) {
                    ty = new TuitionByYear();
                    ty.setId(tyId);
                    ty.setSubject(subj);
                    ty.setTuition(seed.tuition);
                    ty.setCreator(creatorAdmin);
                    em.persist(ty);
                    System.out.println("Added Tuition: " + seed.id + "@" + currentYear + " = " + seed.tuition);
                } else {
                    if (!Objects.equals(ty.getTuition(), seed.tuition)) {
                        ty.setTuition(seed.tuition);
                        System.out.println("Updated Tuition: " + seed.id + "@" + currentYear + " = " + seed.tuition);
                    }
                }
            }
        }
