...
@Repository
@Slf4j
public class EmailServiceForDeputyStaffDAOImpl implements EmailServiceForDeputyStaffDAO {
    ...
    private String generateEmailTemplate(
        ...
        String title = template.getSalutation() != null ? template.getSalutation() : "Access Your Deputy Staff Account";
        String subtitle = template.getGreeting() != null ? template.getGreeting() : "Welcome to Our University Team";
        String mainMessage = template.getBody() != null ? template.getBody() : "Your deputy staff account has been successfully created. Below are your account details and important information.";
        String preheader = template.getGreeting() != null ? template.getGreeting() : "Welcome to our team — your account details and next steps inside.";
        ...
        html.append("<table class='info-table' role='presentation' width='100%' cellpadding='0' cellspacing='0' style='border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;'>")
                ...
                .append("<td style='padding:12px 14px;font-size:14px;color:#374151;border-bottom:1px solid #e5e7eb;font-family:sans-serif;'>").append(safe.apply(context.deputyStaffId())).append("</td>")
                .append("</tr>")
                .append("<tr>")
                ...
                .append("<td style='padding:12px 14px;font-size:14px;color:#374151;border-bottom:1px solid #e5e7eb;font-family:sans-serif;'>").append(safeDate.apply(context.createdDate())).append("</td>")
                .append("</tr>")
                .append("<tr>")
                .append("<td style='padding:12px 14px;font-size:14px;color:#1f2937;font-weight:600;background:#ffffff;font-family:sans-serif;'>Created By</td>")
                .append("<td style='padding:12px 14px;font-size:14px;color:#374151;background:#ffffff;font-family:sans-serif;'>").append(creator).append("</td>")
                .append("</tr>")
                .append("</table>");
        ...
        if (includeCredentials) {
            html.append("<div style='margin:20px 0;padding:16px 16px;border:1px solid #dbeafe;background:#eff6ff;border-radius:10px;'>")
                    ...
                    .append("<div><strong>Username:</strong> ").append(safe.apply(deputyStaffId)).append("</div>")
    ...
    private void sendEmailWithTemplate(
        ...
        log.info("Successfully sent {} email to: {}", templateType, to);
    }
    ...
    public void sendEmailToNotifyLoginInformation(String to, String subject, DeputyStaffEmailContext context, String rawPassword) throws MessagingException {
        try {
            sendEmailWithTemplate(to, subject, context, EmailTemplateTypes.MINOR_LECTURE_ADD, true, context.deputyStaffId(), rawPassword);
        } catch (MessagingException e) {
            log.error("Failed to send login information email to: {}", to, e);
            throw e;
        }
    }
    ...
    public void sendEmailToNotifyInformationAfterEditing(String to, String subject, DeputyStaffEmailContext context) throws MessagingException {
        try {
            sendEmailWithTemplate(to, subject, context, EmailTemplateTypes.MINOR_LECTURE_EDIT, false, "", "");
        } catch (MessagingException e) {
            log.error("Failed to send edit notification email to: {}", to, e);
            throw e;
        }
    }
    ...
    public void sendEmailToNotifyDeputyStaffDeletion(String to, String subject, DeputyStaffEmailContext context) throws MessagingException {
        try {
            sendEmailWithTemplate(to, subject, context, EmailTemplateTypes.MINOR_LECTURE_DELETE, false, "", "");
        } catch (MessagingException e) {
            log.error("Failed to send deletion notification email to: {}", to, e);
            throw e;
        }
    }
}