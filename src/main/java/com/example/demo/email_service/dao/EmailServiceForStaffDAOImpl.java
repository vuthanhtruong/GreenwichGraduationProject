...
@Repository
@Slf4j
public class EmailServiceForStaffDAOImpl implements EmailServiceForStaffDAO {
    ...
    private String generateEmailTemplate(
        ...
        String title = template.getSalutation() != null ? template.getSalutation() : "Access Your Staff Account";
        String subtitle = template.getGreeting() != null ? template.getGreeting() : "Welcome to Our University Team";
        String mainMessage = template.getBody() != null ? template.getBody() : "Your staff account has been successfully created. Below are your account details and important information.";
        ...
        html.append("<table class='info-table' role='presentation' width='100%' cellpadding='0' cellspacing='0' style='border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;'>")
                ...
                .append("<td style='padding:12px 14px;font-size:14px;color:#1f2937;width:40%;font-weight:600;border-bottom:1px solid #e5e7eb;font-family:sans-serif;'>Staff ID</td>")
                .append("<td style='padding:12px 14px;font-size:14px;color:#374151;border-bottom:1px solid #e5e7eb;font-family:sans-serif;'>").append(safe.apply(context.staffId())).append("</td>")
                .append("</tr>")
                .append("<tr>")
                ...
                .append("</tr>")
                .append("<tr style='background:#f8fafc;'>")
                .append("<td style='padding:12px 14px;font-size:14px;color:#1f2937;font-weight:600;border-bottom:1px solid #e5e7eb;font-family:sans-serif;'>Department</td>")
        ...
        if (includeCredentials) {
            html.append("<div style='margin:20px 0;padding:16px 16px;border:1px solid #dbeafe;background:#eff6ff;border-radius:10px;'>")
                    ...
                    .append("<div><strong>Username:</strong> ").append(safe.apply(staffId)).append("</div>")
    ...
    public void sendEmailToNotifyLoginInformation(String to, String subject, StaffEmailContext context, String rawPassword) throws MessagingException {
        try {
            sendEmailWithTemplate(to, subject, context, EmailTemplateTypes.STAFF_ADD, true, context.staffId(), rawPassword);
    ...
    public void sendEmailToNotifyInformationAfterEditing(String to, String subject, StaffEmailContext context) throws MessagingException {
        try {
            sendEmailWithTemplate(to, subject, context, EmailTemplateTypes.STAFF_EDIT, false, "", "");
        } catch (MessagingException e) {
    ...
    public void sendEmailToNotifyStaffDeletion(String to, String subject, StaffEmailContext context) throws MessagingException {
        try {
            sendEmailWithTemplate(to, subject, context, EmailTemplateTypes.STAFF_DELETE, false, "", "");
        } catch (MessagingException e) {
